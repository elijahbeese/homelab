...
| Device | Role |
| --- | --- |
| Dell OptiPlex 7010 | Proxmox hypervisor — runs all VMs |
| Raspberry Pi 5 (8GB) | Pi-hole DNS + Zeek network monitor |
| TP-Link TL-SG108E | Managed switch for VLAN segmentation |
| MacBook Air M2 | Management workstation |
...
| VM ID | OS | Purpose |
| --- | --- | --- |
| 100 | Kali Linux 2025.4 | Offensive security / penetration testing |
| 200 | OPNsense 25.1 | Firewall, routing, VPN, IDS |
| TBD | Splunk | SIEM / log aggregation |
| TBD | Windows Server | Active Directory lab |
| TBD | Metasploitable | Vulnerable target for practice |
...
- [x] Installed **Proxmox VE 9.1** on bare metal (OptiPlex 7010)
- [x] Deployed **OPNsense 25.1** as virtual firewall/router
- [x] Deployed **Kali Linux 2025.4** VM for offensive ops
- [x] Configured virtual networking between VMs

### Phase 2 — Remote Access

- [x] Configured **WireGuard VPN** on OPNsense (instance + peers + firewall rules)
- [x] Installed **Tailscale** on Proxmox host and Kali VM
- [x] Remote access to Proxmox web UI from anywhere via Tailscale
- [x] Remote SSH into Kali from anywhere via Tailscale
...
| Service | Address |
| --- | --- |
| Proxmox Web UI | `https://100.90.195.73:8006` |
| Kali SSH | `ssh elijah@100.72.251.62` |
| OPNsense Web UI | `https://192.168.1.1` (LAN only) |
...
| Tool | Purpose |
| --- | --- |
| OPNsense | Firewall, NAT, WireGuard VPN |
| Pi-hole | DNS filtering, ad blocking, query logging |
| Zeek | Passive network traffic analysis |
| Wazuh | SIEM, HIDS, log aggregation |
| Tailscale | Zero-config remote access mesh VPN |
